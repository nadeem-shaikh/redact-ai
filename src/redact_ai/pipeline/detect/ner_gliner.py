...
_INSTALL_HINT = (
    ...
    "`pip install redact-ai[strong]`. The model is fetched on first load and "
    "cached on-device (ADR-002/ADR-013)."
)


@lru_cache(maxsize=2)
def _load_gliner(model_name: str) -> Any:
    ...
    try:
        ...
        # Any load/resolution failure fails closed with an install hint.
        raise RuntimeError(
            f"GLiNER model '{model_name}' could not be loaded. {_INSTALL_HINT}"
        ) from exc
