...
def test_matched_text_hash_omitted_for_empty_text() -> None:
    ...
    vision = Finding(
        ...
        confidence="medium",
        matched_text="",  # vision detectors have no text content
        page_index=0,
    )
    manifest = build_manifest(
        policy=policy,
    ...
    assert manifest.findings[0].matched_text_hash is None


def test_matched_text_hash_stable_across_runs() -> None:
    policy = load_default_policy()
    findings = [_finding("ID-001", "IDENTITY", 0, 0, 7)]
    a = build_manifest(
        policy=policy,
        runtime_version="0.1.0",
        ocr_engine="tesseract-5.3.4",
    ...
    assert a.findings[0].matched_text_hash == b.findings[0].matched_text_hash
